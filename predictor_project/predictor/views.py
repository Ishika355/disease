...
def diabetes(request):
    ...
    if request.method == 'POST':
        ...
        # Create a ColumnTransformer to one-hot encode categorical variables
        preprocessor = ColumnTransformer(
            transformers=[
                ('num', StandardScaler(), [1, 2, 3, 5, 6, 7]),  # numerical columns
                ('cat', OneHotEncoder(), categorical_indices)  # categorical columns
            ],
            remainder='passthrough'  # passthrough the remaining columns
        )

        # Fit and transform the user data
        user_data_encoded = preprocessor.fit_transform(user_data)

        predictions = grid_search.predict(user_data_encoded)
        print('predictions')
        print(predictions)
        if int(predictions[0]) == 1:
            value = 'have'  # User is predicted to have heart disease
